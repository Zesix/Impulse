...
namespace SpaceShooter2D
{
    ...
    public class FireProjectile : MonoBehaviour
    {
        ...
        virtual protected void FireMountPoint(Transform mountPoint, float speed, Vector3 direction, int MountPointIndex)
        {
            // Create bullet
            Rigidbody2D shot;
            shot = ((GameObject)Instantiate(projectile, mountPoint.position, mountPoint.rotation)).GetComponent<Rigidbody2D>();
            ...
            shot.AddForce(mountPoint.up * speed + direction, ForceMode2D.Impulse);
