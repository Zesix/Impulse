...
{
    // A modified BaseAIController for use in 2D.
    public class BaseAIController : ExtendedMonoBehaviour
    {
        ...
        public virtual int IsObstacleAhead()
        {
            ...
            Debug.DrawRay(MyTransform.position, ((MyTransform.up + (MyTransform.right * 0.5f)) * WallAvoidDistance));
            Debug.DrawRay(MyTransform.position, ((MyTransform.up + (MyTransform.right * -0.5f)) * WallAvoidDistance));

            // cast a ray out forward from our AI and put the 'result' into the variable named hit
            if (Physics.Raycast(MyTransform.position, MyTransform.up + (MyTransform.right * 0.5f), out _hit, WallAvoidDistance))
            {
                // obstacle
        ...
        public void TurnTowardTarget(Transform aTarget)
        {
            ...
            RotateTransform.Rotate(0, 0, _targetAngle * ModelRotateSpeed * Time.deltaTime);
        }
        ...
        public bool CanSee(Transform aTarget)
        {
            ...
            Debug.DrawLine(MyTransform.position, _tempDirVec);

            // cast a ray from our AI, out toward the target passed in (use the tempDirVec magnitude as the distance to cast)
            if (Physics.Raycast(MyTransform.position + (VisionHeightOffset * MyTransform.forward), _tempDirVec, out _hit, MaxChaseDistance))
            {
        ...
        {
            return CurrentAiState == AIState.BackingUpLookingForTarget;
        }
    }
}
