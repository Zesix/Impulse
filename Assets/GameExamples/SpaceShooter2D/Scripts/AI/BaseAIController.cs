...
namespace SpaceShooter2D
{
    ...
    public class BaseAIController : ExtendedMonoBehaviour
    {
        ...
        public virtual int IsObstacleAhead()
        {
            ...
            if (Physics.Raycast(myTransform.position, myTransform.up + (myTransform.right * 0.5f), out hit, wallAvoidDistance))
            {
                // obstacle
        ...
        public void TurnTowardTarget(Transform aTarget)
        {
            ...
            rotateTransform.Rotate(0, 0, targetAngle * modelRotateSpeed * Time.deltaTime);
        }
        ...
        public bool CanSee(Transform aTarget)
        {
            ...
            Debug.DrawLine(myTransform.position, tempDirVec);

            // cast a ray from our AI, out toward the target passed in (use the tempDirVec magnitude as the distance to cast)
            if (Physics.Raycast(myTransform.position + (visionHeightOffset * myTransform.forward), tempDirVec, out hit, maxChaseDistance))
            {
